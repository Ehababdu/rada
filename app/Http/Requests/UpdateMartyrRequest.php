...
class UpdateMartyrRequest extends FormRequest
{
    ...
    {
        // Ensure only authenticated users can update martyrs
        return auth()->check();
    }
    ...
    {
        // Get the martyr ID from the route parameter
        $martyrId = $this->route('martyr') instanceof \App\Models\Martyr
            ? $this->route('martyr')->id
            : $this->route('martyr');
