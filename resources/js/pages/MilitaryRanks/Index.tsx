...
export default function Index({ militaryRanks, filters }: Props) {
    ...
    const columns = useMemo<ColumnDef<MilitaryRank, any>[]>(() => [
        ...
        columnHelper.accessor('is_active', {
            ...
            cell: (info) => (
                <Badge variant={info.getValue() ? "default" : "destructive"} className={cn(
                    info.getValue() ? "bg-green-500/10 text-green-600 border-green-500/20" : ""
                )}>
                    ...
                    <DropdownMenuContent align={isRTL ? "start" : "end"} className="w-40">
                        {can('canViewDetails') && (
                            <DropdownMenuItem asChild>
                                <Link href={`/military-ranks/${info.row.original.id}`}>
                        ...
                        {can('canUpdate') && (
                            <DropdownMenuItem asChild>
                                <Link href={`/military-ranks/${info.row.original.id}/edit`}>
    ...
    const handleSearchChange = (value: string) => {
        ...
        searchTimeoutRef.current = setTimeout(() => triggerSearch(value, filters.is_active), 400);
    };
    ...
    const handleDelete = (id: number) => {
        if (confirm(t('confirm_delete_military_rank'))) {
            router.delete(`/military-ranks/${id}`, {
                onSuccess: () => toast({ title: t('military_rank_deleted_successfully') }),
            });
        }
    };
            ...
            <div className="flex flex-col gap-6 p-6 max-w-[1600px] mx-auto w-full" dir={isRTL ? 'rtl' : 'ltr'}>
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-card p-6 rounded-xl border shadow-sm">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold tracking-tight">{t('military_ranks.title')}</h1>
                            <p className="text-muted-foreground text-sm">{t('military_ranks.manage_ranks')}</p>
                        </div>
                    </div>
                    {can('canCreate') && (
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="flex flex-1 items-center gap-2 min-w-[300px]">
                        ...
                        <Select onValueChange={handleStatusChange} defaultValue={filters.is_active || "all"}>
                            <SelectTrigger className="w-[180px]">
                                <SelectValue placeholder={t('military_ranks.all_status')} />
                            </SelectTrigger>
                            <SelectContent>
                                <SelectItem value="all">{t('military_ranks.all_status')}</SelectItem>
                        ...
                        {(searchTerm || filters.is_active) && (
                            <Button variant="ghost" onClick={() => { setSearchTerm(''); triggerSearch('', ''); }} className="text-muted-foreground">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" size="sm">
                                <Settings2 className={cn("h-4 w-4", isRTL ? "ml-2" : "mr-2")} />
                                {t('show_columns')}
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align={isRTL ? "start" : "end"} className="w-56">
                            <DropdownMenuLabel>{t('columns_visibility')}</DropdownMenuLabel>
                            <DropdownMenuSeparator />
                            ...
                            {table.getHeaderGroups().map((headerGroup) => (
                                <TableRow key={headerGroup.id}>
                                    {headerGroup.headers.map((header) => (
                                        <TableHead key={header.id} className={cn("py-4", isRTL ? "text-right" : "text-left")}>
                ...
                {militaryRanks.last_page > 1 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-t pt-4">
                        <div className="text-sm text-muted-foreground order-2 sm:order-1">
                            {t('showing')} <span className="font-bold text-foreground">{militaryRanks.from}</span> {t('to')} <span className="font-bold text-foreground">{militaryRanks.to}</span> {t('of')} <span className="font-bold text-foreground">{militaryRanks.total}</span> {t('records')}
                        </div>
                        <div className="flex items-center gap-2 order-1 sm:order-2">
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={() => router.get('/military-ranks', { page: militaryRanks.current_page - 1, search: searchTerm, is_active: filters.is_active })}
                            ...
                            <div className="flex items-center gap-1 mx-2">
                                <Badge variant="outline" className="h-8 min-w-[32px] justify-center">
                                    {militaryRanks.current_page}
                                </Badge>
                                <span className="text-muted-foreground">/</span>
                                <span className="text-sm font-medium">{militaryRanks.last_page}</span>
                            </div>
                            ...
                            <Button
                                ...
                                size="sm"
                                onClick={() => router.get('/military-ranks', { page: militaryRanks.current_page + 1, search: searchTerm, is_active: filters.is_active })}
                                disabled={militaryRanks.current_page === militaryRanks.last_page}
                            >
                                {t('next')}
