...
// Icons
import { ArrowLeft, ArrowRight, MapPin, Loader2, Building2, Save, Edit3 } from 'lucide-react';
...
export default function Edit({ bank, branch }: Props) {
    const { t, i18n } = useTranslation();
    const isRTL = i18n.language === 'ar';

    // استخدام useForm لإدارة البيانات والحالة
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: t('branches.edit'), href: `/banks/${bank.id}/branches/${branch.id}/edit` },
    ];

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/banks/${bank.id}/branches/${branch.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${t('branches.edit')} - ${branch.name_ar}`} />
            ...
            <div className="flex flex-col gap-6 p-6 max-w-2xl mx-auto w-full" dir={isRTL ? 'rtl' : 'ltr'}>
                ...
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="icon" asChild className="rounded-full">
                        <Link href={`/banks/${bank.id}/branches/${branch.id}`}>
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground text-sm">
                            {t('branches.edit_branch_description')}
                        </p>
                    </div>
                </div>
                ...
                <Card className="border-none shadow-md overflow-hidden">
                    <CardHeader className="border-b bg-muted/20">
                        <div className="flex items-center justify-between">
                            ...
                            <Badge variant="outline" className="flex items-center gap-1 font-medium italic">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="name_ar" className="text-sm font-semibold">
                                    {t('branches.name_ar')} <span className="text-destructive">*</span>
                                </Label>
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        placeholder={t('branches.enter_name_ar')}
                                        required
                                    />
                                </div>
                                {errors.name_ar && (
