...
export default function Index({ banks, filters }: Props) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [{ title: t('banks.title'), href: '/banks' }];
        ...
        columnHelper.accessor('is_active', {
            ...
            cell: (info) => (
                <Badge variant={info.getValue() ? "default" : "secondary"} className="gap-1">
                    ...
                    {info.getValue() ? t('banks.active') : t('banks.inactive')}
                </Badge>
            ),
        }),
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align={isRTL ? "start" : "end"} className="w-44">
                        ...
                        <DropdownMenuItem asChild>
                            <Link href={`/banks/${info.row.original.id}`} className="cursor-pointer">
                                <Eye className={cn("h-4 w-4", isRTL ? "ml-2" : "mr-2")} /> {t('view')}
                            </Link>
                        </DropdownMenuItem>
                        <DropdownMenuItem asChild>
                            <Link href={`/banks/${info.row.original.id}/edit`} className="cursor-pointer">
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-6 p-6 max-w-7xl mx-auto w-full" dir={isRTL ? 'rtl' : 'ltr'}>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight">{t('banks.title')}</h1>
                        <p className="text-muted-foreground mt-1">{t('banks.manage_banks')}</p>
                    </div>
                    {can('canCreate') && (
                ...
                <Card className="border-none shadow-none bg-muted/40">
                    <CardContent className="p-4 flex flex-col md:flex-row gap-4">
                        ...
                        <div className="flex gap-2">
                            ...
                            <Button variant="outline" size="icon" onClick={() => router.get('/banks')} className="shrink-0 bg-background">
                ...
                {banks.last_page > 1 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4">
                        <p className="text-sm text-muted-foreground italic">
                            {t('showing')} <span className="font-medium text-foreground">{banks.from}</span> {t('to')} <span className="font-medium text-foreground">{banks.to}</span> {t('of')} <span className="font-medium text-foreground">{banks.total}</span>
                        </p>
                        <div className="flex items-center gap-2">
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={() => performSearch({ ...filters, page: banks.current_page - 1 })}
                            ...
                            <Button
                                ...
                                size="sm"
                                onClick={() => performSearch({ ...filters, page: banks.current_page + 1 })}
                                disabled={banks.current_page === banks.last_page}
                            >
                                {t('next')}
                        ...
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription className={cn(isRTL ? "text-right" : "text-left")}>
                                ...
                                <span className="text-xs font-semibold text-muted-foreground mt-2 inline-block">
                                    هذا الإجراء سيقوم بحذف كافة البيانات المرتبطة بهذا البنك بشكل نهائي.
                                </span>
                            </AlertDialogDescription>
                        </AlertDialogHeader>
