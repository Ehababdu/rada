...
export function useBulkSelection<TData>(): UseBulkSelectionReturn<TData> {
    ...
    const toggleAll = useCallback((rows: TData[], getId: (row: TData) => string) => {
        setSelectedIds(prev => {
            const allIds = new Set(rows.map(getId));
            const currentlySelected = rows.filter(row => prev.has(getId(row))).length;
    ...
    const selectRows = useCallback((rows: TData[], getId: (row: TData) => string) => {
        setSelectedIds(new Set(rows.map(getId)));
    }, []);

    return {
        selectedRows: Array.from(selectedIds).map(id =>
            selectedRows.find(row => (row as any).id === id)
